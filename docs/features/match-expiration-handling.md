...
## Description
Implements the 'Match Expiration Handling' capability according to Section 10 – Like / Pass Actions and Match Creation, preserving Night Vibe core invariants (venue-gated interaction, server-side authority, and deterministic conflict handling).

## Scope
- In scope: domain behavior, validation rules, state transitions, and API/event handling required for 'Match Expiration Handling'.
