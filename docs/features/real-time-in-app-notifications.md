...
## Description
Implements the 'Real-Time In-App Notifications' capability according to Section 13 – Notifications and Real-Time Event Delivery, preserving Night Vibe core invariants (venue-gated interaction, server-side authority, and deterministic conflict handling).

## Scope
- In scope: domain behavior, validation rules, state transitions, and API/event handling required for 'Real-Time In-App Notifications'.
