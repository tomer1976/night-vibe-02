...
## Description
Implements the 'Role-Based Access Control (RBAC)' capability according to Section 3 – Stakeholders and User Roles, preserving Night Vibe core invariants (venue-gated interaction, server-side authority, and deterministic conflict handling).

## Scope
- In scope: domain behavior, validation rules, state transitions, and API/event handling required for 'Role-Based Access Control (RBAC)'.
